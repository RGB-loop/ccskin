...
## 0. bytecode 构建上改 JS 源码文本是无效的(最大的坑)
2.1.26x 起 claude 用 `bun build --bytecode` 编译。二进制里那份 JS 源码
**文本还在**,但运行时读的是 bytecode 常量池 —— 改源码文本能通过锚点计数、
sha1、`--version` 全部校验,**屏幕上却毫无变化**。
判定:文件里出现 `// @bun @bytecode` 即为 bytecode 构建。
→ `discovery.build_patches` 检测到就整体切到常量池通道(`bytecode.py`),
`verify` 也改成查常量池;老版本(非 bytecode)仍走源码文本通道。

常量池条目布局:
```
<len:  u32 LE,高位 0x80000000 置位 = 8-bit(latin1),否则 UTF-16>
<hash: u32 LE(实际 24 位);运行时不校验,改内容不必改它>
<chars>
<pad:  补 0 到 4 字节对齐>      ← 不是终止符,当成终止符解析会漏掉一半条目
```

## 0b. 常量池按内容去重,一条可能被多处共用
池里内容相同的字面量只有一条。后果:
- **图标**: 上游自己就让 `default` 与 `arms-up` 共用 r1E,设计稿给它们不同
  图案物理上做不到 → `_pool_icon_map` 检测冲突并报错,不静默取其一。
- **版本号**: 启动画面的版本与 `--version` 共用同一条常量,没法只改显示
  → 默认跳过版本位;要强改得显式开 `rewrite_real_version`。
- **名字**: `Claude Code` 是一条,但 `--version` 里的 ` (Claude Code)`
  与机器标识 `claude-code` 是**另外的**条目,不受影响。
...
## 3. 源码文本区与常量池区要分清
同一个字符串在二进制里有两份: 源码文本区(\uXXXX 转义形式)和 bytecode
常量池(UTF-16 / latin1 裸字符)。**bytecode 构建下只有后者驱动 UI**(见 0)。
→ 源码通道只认完整结构锚点(槽位正则 + 括号配对);常量池通道用
`bytecode.find()` 按条目边界精确匹配,短串再用 `region_for()` 限定区间消歧。
